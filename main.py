...
# -----------------------------------------------------------------
# MAIN INTERFACE CORE - Pure String Format (No f-string fumbles!)
# -----------------------------------------------------------------
RAW_UI_HTML = """
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <script>
        ...
        // Safely injected via clean string replacement instead of dynamic f-string curly blocks
        const nativeInput = "STREAMLIT_INPUT_VAR".toLowerCase().trim();
...
# Replace the internal placeholder string safely with the current Streamlit text state
CLEANED_UI_HTML = RAW_UI_HTML.replace("STREAMLIT_INPUT_VAR", voice_input)

# Render the application layout container completely risk-free
components.html(CLEANED_UI_HTML, height=620, scrolling=False)
